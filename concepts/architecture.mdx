...
<CardGroup cols={2}>
  ...
  <Card title="HTTP API" icon="globe">
    An Express server on port 7777. Exposes `POST /send`, `GET /events` (SSE), `/workers`, `/memories`, `/skills`, and more. Authentication via bearer token stored in `~/.max/api-token`.
  </Card>
...
<Steps>
  <Step title="Message arrives">
    A message arrives from Telegram, the TUI (via `POST /send`), or a background worker completion. It is tagged with its source channel (`[via telegram]`, `[via tui]`, or left untagged for background).
  </Step>
  <Step title="Enqueued">
