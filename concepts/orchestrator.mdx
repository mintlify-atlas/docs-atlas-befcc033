...
<Note>
  Auto-routing is **disabled by default**. Enable it with `toggle_auto` or via the `/model auto` TUI command. When disabled, the model configured in `~/.max/.env` is used for every message.
</Note>
